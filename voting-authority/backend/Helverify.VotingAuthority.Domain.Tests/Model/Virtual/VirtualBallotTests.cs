...
{
    internal class VirtualBallotTests
    {
        ...
        [Test]
        public void TestGetSelectedEncryptions()
        {
            // arrange
            string shortCode = "da";
            IElGamal elGamal = new ExponentialElGamal();
            ...
            VirtualBallot virtualBallot = new VirtualBallot(new List<PlainTextOption>(), publicKey)
            {
                EncryptedOptions = new List<EncryptedOption>
                {
                    new (publicKey, new List<int> { 0, 1 }){ShortCode = "f1"},
                    new (publicKey, new List<int> { 1, 0 }){ShortCode = shortCode}
                }
            };

            // act
            IList<EncryptedOption> selection = virtualBallot.GetSelectedEncryptions(new List<string> { shortCode });
            
            // assert
            Assert.That(selection.Count, Is.EqualTo(1));
            Assert.That(selection.Single().ShortCode, Is.EqualTo(shortCode));
        }
