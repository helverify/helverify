using Helverify.ConsensusNode.Domain.Model;
using Helverify.Cryptography.Encryption;
using Org.BouncyCastle.Crypto;
using Org.BouncyCastle.Crypto.Parameters;
using Org.BouncyCastle.Math;

namespace Helverify.ConsensusNode.Domain.Tests.Model
{
    internal class CipherTextTests
    {
        private readonly BigInteger _p = new("",
            16);
        private readonly BigInteger _g = new("", 16);


        [Test]
        public void TestDecrypt()
        {
            // arrange
            IElGamal elGamal = new ExponentialElGamal();

            AsymmetricCipherKeyPair keyPair1 = elGamal.KeyGen(_p, _g);
            AsymmetricCipherKeyPair keyPair2 = elGamal.KeyGen(_p, _g);

            IList<DHPublicKeyParameters> publicKeys = new List<DHPublicKeyParameters>
            {
                (keyPair1.Public as DHPublicKeyParameters)!,
                (keyPair2.Public as DHPublicKeyParameters)!
            };

            DHPublicKeyParameters combinedPublicKey = elGamal.CombinePublicKeys(publicKeys, new DHParameters(_p, _g));
            ElGamalCipher elGamalCipher = elGamal.Encrypt(1, combinedPublicKey);

            CipherText cipherText = new CipherText(elGamalCipher);

            // act
            DecryptedShare decryptedShare1 = cipherText.Decrypt(keyPair1);
            DecryptedShare decryptedShare2 = cipherText.Decrypt(keyPair2);

            // assert
            string share1 = decryptedShare1.Share;
            string share2 = decryptedShare2.Share;
            List<BigInteger> shares = new List<BigInteger> { new (share1, 16), new (share2, 16) };

            Assert.That(elGamal.CombineShares(shares, elGamalCipher.D, _p, _g), Is.EqualTo(1));
            Assert.That(decryptedShare1.ProofOfDecryption.Verify(elGamalCipher.C, elGamalCipher.D, keyPair1.Public as DHPublicKeyParameters), Is.True);
            Assert.That(decryptedShare2.ProofOfDecryption.Verify(elGamalCipher.C, elGamalCipher.D, keyPair2.Public as DHPublicKeyParameters), Is.True);
        }
    }
}
